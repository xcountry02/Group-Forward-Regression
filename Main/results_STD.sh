...
do
  ...
  do
    ...
    do
      ...
      do
        # echo /gsfs1/xdisk/kamichels/gFRmpi/Program/Data/Main/print $p0 $n $g $s $l $v
        /gsfs1/xdisk/kamichels/gFRmpi/Program/Data/Main/print $p0 $n $g $s $l $v 0
        (mpirun -np $nodes ./gFRmpi STD Y$l.$p0.$g.$n.${s}_${v} X$l.$p0.$g.$n.${s}_${v} 0 $csv $v $s > "$p0$n$g$s$m$i$v$l.out") >& myerror
